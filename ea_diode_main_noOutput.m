function [roll_out,solution] = ea_diode_main_noOutput(options,supervised,experiment)
       ...
       for side = options.elside
        ...
        if ~supervised
            ...
            for k = checkslices
                ...
                [~, intensity,~] = ea_orient_intensityprofile(tmp,center_tmp,pixdim,radius);
...
%                     rollangles{2}(count,k) = rolltemp;
            ...
            [anglenew, intensitynew,vectornew] = ea_orient_intensityprofile(artifact_dirnew,center_dirnew,pixdim,radius);
...
%             rollangles = rollangles(darkstarslice,:);
        ...
        %% No Output changes downstream
        % uiwait
        SaveButton.UserData = 1;
        ...
        %% saving results
        if savestate == 1
            export_fig(['J:\Till\DiODeV2\' experiment '\PNGS\' options.patientname '_' num2str(side) '_' sides{side} '.png'])
            clear x y
